...
foreach ($vehicles as &$vehicle) { $vehicle['status'] = updateVehicleStatus($pdo, $vehicle['id']); }
...
<head>
    ...
    <style>
        .page-header-info { background: linear-gradient(135deg, #3b82f6, #60a5fa); color: white; padding: 20px 28px; border-radius: 14px; margin-bottom: 24px; }
        .page-header-info h2 { font-size: 1.5rem; font-weight: 800; }
        .page-header-info p { opacity: 0.85; margin-top: 4px; }
        .back-btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; background: white; color: #3b82f6; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 0.9rem; border: 2px solid #3b82f6; transition: all 0.2s; margin-bottom: 20px; }
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="sidebar-menu">
                <a href="dashboard.php" class="menu-item">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    Dashboard
                </a>
                ...
                <a href="manage_guards.php" class="menu-item">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    Manage Guards
                </a>
        ...
        <main class="main-content">
            <div class="top-nav">
                <div class="top-nav-left" style="display:flex;align-items:center;gap:12px;">
                    ...
                    <div><h1>Total Vehicles</h1><p>All registered vehicles in the system</p></div>
                </div>
            ...
            <div class="dashboard-content">
                <a href="dashboard.php" class="back-btn">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width:16px;height:16px;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    Back to Dashboard
                </a>
                <div class="page-header-info">
                    <h2>All Vehicles (<?= count($vehicles) ?>)</h2>
                    <p>Complete list of all registered vehicles including valid, expired and revoked.</p>
                </div>
                <div class="data-card">
                    <div class="table-wrapper-modern">
                        <?php if (count($vehicles) > 0): ?>
                        <table class="table-modern">
                            <thead><tr><th>Photo</th><th>Plate No.</th><th>Owner</th><th>Owner Type</th><th>Vehicle Type</th><th>Brand</th><th>Color</th><th>Status</th><th>Registered</th><th>Expiration</th><th>Actions</th></tr></thead>
                            <tbody>
                                <?php foreach ($vehicles as $v): ?>
                                <?php $sc = $v['status']==='Expired'?'status-expired':($v['status']==='Revoked'?'status-revoked':'status-valid'); ?>
                                <tr>
                        ...
                        <?php else: ?><div class="empty-state"><p>No vehicles found.</p></div><?php endif; ?>
                    </div>
                </div>
            </div>
        </main>
